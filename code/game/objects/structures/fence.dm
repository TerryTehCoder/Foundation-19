...
/obj/structure/fence/attackby(obj/item/W, mob/user)
	if(isWirecutter(W))
		...
		if(do_after(user, CUT_TIME, target = src)) //Traditionally Cut_Time is multiplied by toolspeed but I'm not sure if we have defs for that on Bay.
...
/obj/structure/fence/door/proc/toggle(mob/user)
	...
	set_density(!density)
	update_icon()
	playsound(src, 'sounds/structures/fencedoortoggle.ogg', 100, TRUE)

/obj/structure/fence/door/update_icon()
